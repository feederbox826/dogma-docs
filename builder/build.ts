...
class Plugin {
    ...
    author: string // author of collection
    path: string // path to plugin in repository
    screenshots: string[] // screenshots of plugin
    readme: string | undefined // readme file
    ...
    constructor(defaults: LocalCollection, sidecar: LocalSidecar | undefined, index: RemotePlugin) {
        ...
        this.author = defaults?.global_author
            ?? sidecar?.author // overriden author
            ?? "Not Attributed" // no author
        this.path = sidecar?.path
            ?? index.id // default to ID
        this.screenshots = sidecar?.screenshots ?? []
        this.readme = sidecar?.readme // readme file
    ...
    printMD() {
        ...
        const readme = this.readme
            ? `View [README.md](${filePath}/README.md){target=_blank}`
            : "No README"
        const screenshots = this.screenshots
            .map(screenshot => `![${this.name} screenshot](${screenshot})  `)
            .join("\n")
        const author = this.author.includes("http") // if includes link, add target blank
